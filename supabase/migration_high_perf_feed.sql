-- ============================================================================
-- DAK SOCIAL NETWORK - PERSONALIZED FEED SYSTEM MIGRATION
-- Sets up tables, views, index optimizations, and personalized feed logic.
-- ============================================================================

-- 1. ALTER THREADS TABLE TO SUPPORT ALGORITHMIC CLASSIFICATION
ALTER TABLE public.threads ADD COLUMN IF NOT EXISTS category TEXT DEFAULT 'general';
ALTER TABLE public.threads ADD COLUMN IF NOT EXISTS media_hd BOOLEAN DEFAULT false;
ALTER TABLE public.threads ADD COLUMN IF NOT EXISTS is_original BOOLEAN DEFAULT true;
ALTER TABLE public.threads ADD COLUMN IF NOT EXISTS spam_score NUMERIC DEFAULT 0.0;

-- 2. CREATE COMMUNITIES TABLES FOR COMMUNITY POOL
...
CREATE TABLE IF NOT EXISTS public.user_interactions (
    ...
    interaction_type TEXT NOT NULL, -- 'like', 'reply', 'watch_time', 'save', 'click', 'scroll_away', 'hide', 'report'
    duration_seconds INTEGER DEFAULT 0 NOT NULL,
    score NUMERIC DEFAULT 0.0 NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- Optimize queries matching category scores per user
CREATE INDEX IF NOT EXISTS idx_user_interactions_uid_cat ON public.user_interactions(user_id, category);
...
-- 4. CREATE USER FEED CACHE FOR FEED RANKINGS
DROP TABLE IF EXISTS public.user_feed_cache CASCADE;
...
CREATE INDEX IF NOT EXISTS idx_user_feed_cache_user_score ON public.user_feed_cache(user_id, score DESC);

ALTER TABLE public.user_feed_cache DISABLE ROW LEVEL SECURITY;

-- 5. RELATIONSHIP VIEWS FOR SCORING FACTOR
-- Drop views first to prevent column structure/rename errors
DROP VIEW IF EXISTS public.friends_view CASCADE;
DROP VIEW IF EXISTS public.frequent_chats_view CASCADE;

-- Friends View: Mutual Followers
...
-- Frequent Chats View: DM contact frequency in the last 30 days
CREATE OR REPLACE VIEW public.frequent_chats_view AS
SELECT 
...
-- 6. USER INTERACTION LOGGING RPC FUNCTION
...
DECLARE
    ...
    v_score NUMERIC;
BEGIN
    -- Resolve category of the thread
    SELECT COALESCE(category, 'general') INTO v_category FROM public.threads WHERE id = p_thread_id;
    IF v_category IS NULL THEN
        v_category := 'general';
    END IF;

    -- Map points based on user behavior type
    v_score := CASE LOWER(p_type)
        WHEN 'click' THEN 2.0
        WHEN 'like' THEN 5.0
        WHEN 'reply' THEN 8.0
        WHEN 'save' THEN 6.0
        WHEN 'watch_time' THEN COALESCE(p_duration, 0)::NUMERIC / 10.0
        WHEN 'scroll_away' THEN -3.0
        WHEN 'hide' THEN -10.0
        WHEN 'report' THEN -20.0
        ELSE 1.0
    END;

    -- Save log
    INSERT INTO public.user_interactions (user_id, thread_id, category, interaction_type, duration_seconds, score)
    VALUES (p_user_id, p_thread_id, v_category, p_type, COALESCE(p_duration, 0), v_score);
    
    -- If negative interaction, purge from user's active feed cache
    IF LOWER(p_type) IN ('hide', 'report', 'scroll_away') THEN
        DELETE FROM public.user_feed_cache WHERE user_id = p_user_id AND thread_id = p_thread_id;
    END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 7. THE PERSONALIZED RANKING ALGORITHM WITH 5 CANDIDATE POOLS
...
-- 8. TRIGGER TO CLEAR USER FEED CACHE ON NEW POST INSERTION
CREATE OR REPLACE FUNCTION public.clear_feed_cache_on_new_post()
RETURNS TRIGGER AS $$
BEGIN
    DELETE FROM public.user_feed_cache WHERE user_id = NEW.user_id;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE OR REPLACE TRIGGER on_new_thread_clear_cache
AFTER INSERT ON public.threads
FOR EACH ROW EXECUTE FUNCTION public.clear_feed_cache_on_new_post();
